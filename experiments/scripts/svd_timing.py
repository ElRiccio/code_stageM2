...
Run as `python -m experiments.scripts.svd_timing`.
"""
...
from experiments import map_catalogue, svd_timing, tables
...
def main() -> None:
    ...
    csv_path = tables.save_csv(rows, RESDIR, "timing_table.csv")
    ...
    ax.set_title(f"Timing vs. exact SVD, {svd_timing.DEFAULT_M}x{svd_timing.DEFAULT_N}, D={svd_timing.DEFAULT_D}, CPU")
    ax.legend(fontsize=8)
    ax.grid(True, axis="y", linewidth=0.3, alpha=0.5)
    fig.tight_layout()
    os.makedirs(FIGDIR, exist_ok=True)
    fig_path = os.path.join(FIGDIR, "svd_timing.png")
