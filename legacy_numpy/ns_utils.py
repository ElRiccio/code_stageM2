...
def coeffs_psi(d):
    ...
    q[0] -= 1.0
    for _ in range(d + 1):
        q, _ = np.polynomial.polynomial.polydiv(q, np.array([-1.0, 1.0]))
    _CACHE_PSI[d] = q
    return q
...
def constants(d, n_grid):
    ...
    mu = float(np.max(np.abs(psi_d(x, d))))
    rho = mu ** (-1.0 / d)
...
def log10_err_orbit(u0, d, n_iters, switch=1e-8):
    ...
    for k in range(n_iters):
        if e > switch:
            u_next = float(np.clip(p_d(u, d), 0.0, 1.0))
            e_next = 1.0 - u_next
            if e_next > switch:
...
class Spline(object):
    ...
    def ext(self, x):
        ...
        for i in range(len(self.w)):
            out = out + self.w[i] * np.maximum(x - self.interior[i], 0.0)
        return out
...
def sgn_matrix(M, a, n_iters, scale=None, norm_method="svd", norm_kw=None, tol=None):
    ...
    If `tol` is given, the iteration stops early once ||X_{k+1} - X_k||_F < tol.
    Past convergence the null-space directions of a rank-deficient M sit at an
    unstable fixed point (slope a[0] > 1 at 0), so float noise there grows
    geometrically with every extra step; stopping as soon as the orbit settles
    avoids feeding that noise back in. `tol=None` (default) disables the check
    and always runs the full n_iters steps.
    """
    M = np.asarray(M, dtype=float)
    if scale is None:
    ...
    for _ in range(n_iters):
        X_prev = X
        X = iter_matrix(X, a)
        if tol is not None and np.linalg.norm(X - X_prev) < tol:
            break
    return X


def orbit_matrix(M, a, n_iters, scale=None, norm_method="svd", norm_kw=None, tol=None):
    ...
    If `tol` is given, iterating stops early once ||X_{k+1} - X_k||_F < tol
    (see sgn_matrix), and the remaining entries repeat that last X so the
    returned list still has n_iters+1 entries, indexable exactly as before.
    `tol=None` (default) always iterates the full n_iters steps.
    """
    M = np.asarray(M, dtype=float)
    if n_iters < 0:
        raise ValueError("n_iters must be nonnegative")
...
def make_sgn_matrix(d, n_iters, norm_method="svd", norm_kw=None, tol=None):
    ...
    def Sgn(M):
        return sgn_matrix(M, a, n_iters, norm_method=norm_method, norm_kw=norm_kw, tol=tol)
...
def sgn_cfg(cfg, k):
    ...
    return make_sgn_matrix(cfg["d"], int(k), norm_method=cfg.get("norm_method", "svd"),
                           norm_kw=norm_kwargs(cfg), tol=cfg.get("tol"))
...
# Append this block to the end of ns_utils.py.
# The three scalar profiles belong to section 3, the four maps to section 4d;
# nothing here is referenced earlier, so pasting the whole block at the end works.

# ----------------------------------------------------------------------------
# 7. The remaining profiles of the catalogue, and their sign forms
# ----------------------------------------------------------------------------


def lrelu(x, a):
    """Leaky rectifier max(x, a x), for a < 1."""
    x = np.asarray(x, dtype=float)
    return np.maximum(x, a * x)


def crelu(x, a, beta):
    """Capped leaky rectifier min(max(x, a x), beta), for a < 1 and beta > 0."""
    return np.minimum(lrelu(x, a), beta)


def lkc(x, a, mu):
    """Leaky clipping profile a x + (1-a)/2 (|x+mu| - |x-mu|), odd."""
    x = np.asarray(x, dtype=float)
    return a * x + 0.5 * (1.0 - a) * (np.abs(x + mu) - np.abs(x - mu))


def mod_map(M, mu, Sgn, N=None):
    """Matrix modulus Mod_mu(M) = S_mu Sgn(S_mu)^T Sgn(M), S_mu = mu Sgn(M) - M."""
    M = np.asarray(M, dtype=float)
    if N is None:
        N = Sgn(M)
    Mmu = mu * N - M
    return Mmu @ Sgn(Mmu).T @ N


def lrelu_map(M, a, Sgn, N=None):
    """Row four of Tab. forms: on R^{m x n} the sign form does not see the leak."""
    return np.asarray(M, dtype=float)


def crelu_map(M, a, beta, Sgn, N=None):
    """Rows five and six of Tab. forms: (1/2)[beta Sgn(M) + M - Mod_beta(M)]."""
    M = np.asarray(M, dtype=float)
    if N is None:
        N = Sgn(M)
    return 0.5 * (beta * N + M - mod_map(M, beta, Sgn, N))


def lkc_map(M, a, mu, Sgn, N=None):
    """Row seven of Tab. forms: a M + (1-a)/2 [M + mu Sgn(M) - Mod_mu(M)]."""
    M = np.asarray(M, dtype=float)
    if N is None:
        N = Sgn(M)
    return a * M + 0.5 * (1.0 - a) * (M + mu * N - mod_map(M, mu, Sgn, N))

# Append this block to ns_utils.py, after the block of the previous step.
# It adds the symmetric layer: eigenvalue operators, the matrix absolute value,
# and the five symmetric sign forms of Tab. forms, column two.

# ----------------------------------------------------------------------------
# 8. The symmetric layer: eigenvalue operators and the symmetric sign forms
# ----------------------------------------------------------------------------


def calc_eig(M):
    """Eigenvalue decomposition (lam, Q) of a symmetric matrix, lam ascending."""
    lam, Q = np.linalg.eigh(np.asarray(M, dtype=float))
    return lam, Q


def op_eig(M, f):
    """Reference eigenvalue operator Q diag(f(lam)) Q^T of Def. eig-op.

    No odd extension is involved: on Sym^n the profile is read as it stands.
    """
    lam, Q = calc_eig(M)
    return (Q * np.asarray(f(lam), dtype=float)) @ Q.T


def make_S(cfg):
    """Symmetric test matrix, normalized so that ||S||_2 = 1."""
    rng = np.random.default_rng(cfg["seed"])
    n = int(cfg["n"])
    A = rng.standard_normal((n, n))
    S = 0.5 * (A + A.T)
    if cfg.get("normalize", True):
        beta = spec_norm_svd(S)
        if beta > 0.0:
            S = S / beta
    return S
...
def mabs(A, Sgn):
    """Matrix absolute value |A| = A Sgn(A)^T, for A symmetric."""
    A = np.asarray(A, dtype=float)
    return A @ Sgn(A).T


def clip_map_sym(M, alpha, beta, Sgn):
    """Tab. forms, clip: (1/2)[(a+b)I + |M - aI| - |M - bI|]."""
    M = np.asarray(M, dtype=float)
    I = np.eye(M.shape[0])
    return 0.5 * ((alpha + beta) * I + mabs(M - alpha * I, Sgn)
                  - mabs(M - beta * I, Sgn))


def soft_map_sym(M, gamma, Sgn):
    """Tab. forms, soft: M + (1/2)[|M - gI| - |M + gI|]."""
    M = np.asarray(M, dtype=float)
    I = np.eye(M.shape[0])
    return M + 0.5 * (mabs(M - gamma * I, Sgn) - mabs(M + gamma * I, Sgn))


def lrelu_map_sym(M, a, Sgn):
    """Tab. forms, leaky rectifier: (1/2)[(1+a)M + (1-a)|M|]."""
    M = np.asarray(M, dtype=float)
    return 0.5 * ((1.0 + a) * M + (1.0 - a) * mabs(M, Sgn))


def crelu_map_sym(M, a, beta, Sgn):
    """Tab. forms, capped leaky rectifier:
    (1/2)[bI + aM + (1-a)|M| - |M - bI|]."""
    M = np.asarray(M, dtype=float)
    I = np.eye(M.shape[0])
    return 0.5 * (beta * I + a * M + (1.0 - a) * mabs(M, Sgn)
                  - mabs(M - beta * I, Sgn))


def lkc_map_sym(M, a, mu, Sgn):
    """Tab. forms, leaky clipping: aM + (1-a)/2 [|M + uI| - |M - uI|]."""
    M = np.asarray(M, dtype=float)
    I = np.eye(M.shape[0])
    return a * M + 0.5 * (1.0 - a) * (mabs(M + mu * I, Sgn)
                                      - mabs(M - mu * I, Sgn))

# Append this block to ns_utils.py. It is independent of the two earlier
# blocks: it adds the truncated series B_D, the radius R_D of Prop. basin,
# and the iteration count K_D of Cor. count.

# ----------------------------------------------------------------------------
# 9. The basin of attraction and the iteration count
# ----------------------------------------------------------------------------


def bser_coeffs(D):
    """Ascending coefficients c_0,...,c_D of the truncated series B_D."""
    return np.array([cbin(j) / 4.0**j for j in range(D + 1)])


def bser(t, D):
    """B_D(t) = sum_{j<=D} c_j t^j, the truncation of (1-t)^{-1/2}."""
    return np.polynomial.polynomial.polyval(np.asarray(t, dtype=float),
                                            bser_coeffs(D))


def radius_D(D):
    """(R_D, t_D) of Prop. basin: Xi_D = B_D for D odd, q_D for D even."""
    if D < 1:
        raise ValueError("the radius is defined for D >= 1")
    c = bser_coeffs(D)
    xi = c if D % 2 == 1 else c[1:]      # ascending coefficients of Xi_D
    r = np.roots(xi[::-1])               # np.roots wants them descending
    real = r[np.abs(r.imag) < 1e-10].real
    neg = real[real < 0.0]
    if neg.size == 0:
        raise RuntimeError("no negative root found for D=%d" % D)
    t_D = float(np.max(neg))             # largest negative real root
    return float(np.sqrt(1.0 - t_D)), t_D


def kcount(D, theta, eps):
    """K_D(theta, eps) of Cor. count, the number of steps the bound demands."""
