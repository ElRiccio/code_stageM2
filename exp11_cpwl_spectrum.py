...
Validates Thm. spline. Only the values on the half-line are ever used, so the
first knot sits at the origin with s(0)=0 as in
eq. net-normalization; then eta = 0, the expression reduces to
c_0 M + sum_i w_i H_{x_i}(M), and no hypothesis beyond that is needed even when
the spectral tail is edited down to zeros. The knot values are nondecreasing, so
the spectral coordinates plotted below are also the singular values of the
output.
