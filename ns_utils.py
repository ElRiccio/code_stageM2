# ns_utils.py
"""Shared numerics for the sign-iteration experiments.

Central object: the coefficient array a of the odd polynomial
    p(x) = sum_{j=0}^{d} a[j] x^(2j+1),
which is read scalarly by p_coeff and, on matrices, by iter_matrix as
    Phi(X) = sum_{j=0}^{d} a[j] (X X^T)^j X.
The canonical family of Prop. family is a = coeffs_a(d).

Section 4 carries the matrix layer used by the numerical experiments: the
pre-scaling norm (exact, or matrix-free by power iteration), random matrices with
a prescribed spectrum, the reference operators read off an SVD, and the
decomposition-free expressions of Thm. clip, Thm. soft, Lem. mhinge and
Thm. spline. Each expression takes the sign map as a callable, so the exact Sgn
and the Newton-Schulz surrogate are substitutable for one another and a new
operator is added by writing one function.

The experiment scripts share their setup through make_M, norm_kwargs, pre_norm
and sgn_cfg, all of which read the experiment's own CFG dictionary.
"""
...
def p_coeff(x, a):
    ...
    Same arithmetic path as iter_matrix, hence the right evaluator whenever the
    scalar run must reproduce the matrix run exactly.
    """
    x = np.asarray(x, dtype=float)
    x2 = x * x
...
def coeffs_psi(d):
    ...
    q[0] -= 1.0
    # Deflate one linear factor at a time: dividing by the expanded (x-1)^(d+1)
    # would cancel its large binomial coefficients against each other.
    for _ in range(d + 1):
...
def log10_err_orbit(u0, d, n_iters, switch=1e-8):
    """log10|1 - u_k| for u_0 in (0,1), hybrid float / log-space propagation.

    While the error is O(1) it is read off the float orbit; once it falls below
    `switch` the float value of 1-u_k is dominated by cancellation and, a few
    steps later, underflows, so the exact recursion
    e_{k+1} = e_k^(d+1) |psi_d(u_k)| is continued additively in log space.
    """
    u0 = float(u0)
    if not 0.0 < u0 < 1.0:
...
def spec_norm_power(M, n_iters=200, tol=1e-12, seed=0, pad=1.0 + 1e-6):
    ...
    Power iteration converges from below while p_d diverges above 1, so an
    unpadded estimate can leave the top singular value outside the invariant
    interval; `pad` buys the inequality beta >= ||M||_2 of Lem. prescaling.
    """
    M = np.asarray(M, dtype=float)
    if M.size == 0:
        return 0.0
...
def spectrum_log(r, s_min, s_max, n_zero=0):
    """Descending spectrum: r-n_zero log-spaced values in [s_min,s_max], then zeros.

    Log spacing spreads the burn-in of Prop. burnin across the spectrum, and the
    exact zeros make the argument genuinely rank deficient.
    """
    r = int(r)
    n_zero = int(n_zero)
    if r < 1:
    ...
    if not 0 <= n_zero <= r:
        raise ValueError("require 0 <= n_zero <= r")
    sig = np.zeros(r)
    n_pos = r - n_zero
    if n_pos > 0:
        sig[:n_pos] = np.logspace(math.log10(s_max), math.log10(s_min), n_pos)
    return sig


def rand_spec(m, n, sigma, rng):
    """M = U diag(sigma) V^T with random semi-orthogonal factors."""
    sigma = np.asarray(sigma, dtype=float).ravel()
    r = min(m, n)
    if sigma.size != r:
        raise ValueError("sigma must carry min(m,n) entries")
    U = rand_orth(m, r, rng)
    V = rand_orth(n, r, rng)
    return (U * sigma) @ V.T
...
def make_M(cfg):
    """Test matrix of an experiment, read off its CFG dictionary.

    cfg['gen'] is 'spec', which prescribes the spectrum and is the only way to
    impose exact zeros and hence genuine rank deficiency, or 'gauss', which
    samples the entries and leaves the spectrum to Marchenko-Pastur, almost
    surely of full rank. Normalizing afterwards is free by Rem. clip-scaling and
    puts every threshold on the scale of the singular values.
    """
    rng = np.random.default_rng(cfg["seed"])
    m, n = int(cfg["m"]), int(cfg["n"])
    gen = cfg.get("gen", "spec")
    if gen == "spec":
        sig = spectrum_log(min(m, n), cfg["s_min"], cfg["s_max"], cfg.get("n_zero", 0))
        M = rand_spec(m, n, sig, rng)
    elif gen == "gauss":
...
def sweep_op(op, k_list, Y_ex, U, V):
    ...
    `op` maps an iteration count to a matrix, so a new operator enters the
    experiments as a single one-line closure and nothing else changes.
    """
    out = {}
    for k in k_list:
        Y = op(int(k))
...
def plot_spectrum(ax, sig, exact, approx, ks, title, ylabel="spectral coordinate"):
    """Input spectrum (dotted), target (dashed), and the NS values at a few k."""
    idx = np.arange(1, len(np.asarray(sig)) + 1)
    ax.plot(idx, sig, ":", color="0.35", lw=1.2, label=r"$\sigma_i(M)$")
    for k, v in zip(ks, approx):
        ax.plot(idx, v, lw=1.0, label="k=%d" % int(k))
    ax.plot(idx, exact, "k--", lw=1.3, label="exact (SVD)")
    setup_ax(ax, "index i", ylabel, title)
...
if __name__ == "__main__":
    ...
    cfg = {"m": 8, "n": 6, "seed": 0, "gen": "spec", "n_zero": 2,
           "s_min": 1e-2, "s_max": 1.0, "d": 3}
    spl = Spline([0.0, 0.3, 0.7, 1.0], [0.0, 0.2, 0.6, 0.8])
    fc = odd_ext(lambda t: clip_ab(t, -0.2, 0.5))
    for gen in ("spec", "gauss"):
        cfg["gen"] = gen
        Mt = make_M(cfg)
        U, s, V = calc_svd(Mt)
        Sgn = sgn_cfg(cfg, 14)
...
# # ----------------------------------------------------------------------------
# # 3. Iterated sign and the operators built from it
# # ----------------------------------------------------------------------------
...
# # ----------------------------------------------------------------------------
# # 4. Matrix reading of the same coefficients (for the matrix experiments)
# # ----------------------------------------------------------------------------
...
# def sgn_matrix(M, a, n_iters, scale=None):
#     """Approximate matrix sign: n_iters steps of Phi on M pre-scaled to unit norm.
...
#     M = np.array([[0.3]])
#     print("scalar vs matrix reading at x=0.3, d=2, 3 steps: %.3e"
#           % abs(float(sgn_matrix(M, coeffs_a(2), 3)[0, 0])
#                 - float(orbit_pd(0.3, 2, 3)[-1, 0])))